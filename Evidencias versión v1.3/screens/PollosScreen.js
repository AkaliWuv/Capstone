...
import RegistroPollosModal from './RegistroPollosModal'; // Ensure this path is correct
import { Picker } from '@react-native-picker/picker';
import EditPollosModal from './EditPollosModal'; // Ensure this path is correct
...
function PollosScreen() {
  ...
  React.useLayoutEffect(() => {
    navigation.setOptions({
      headerTitle: () => <Text style={styles.headerTitle}>Registro de Pollos</Text>,
  ...
  const confirmDelete = async () => {
    try {
      const response = await fetch(`http://10.0.2.2:5000/api/pollos/${itemToDelete}`, {
        method: 'DELETE',
      });
      if (!response.ok) {
