...
function NivelesComidaScreen() {
    ...
    const [filterType, setFilterType] = useState('fecha_compra'); // Cambiar a 'fecha_compra'
  ...
  React.useLayoutEffect(() => {
    navigation.setOptions({
      headerTitle: () => <Text style={styles.headerTitle}>Niveles de comida</Text>,
  ...
  const getFilteredData = () => {
    ...
    filteredData.sort((a, b) => {
      const valueA = filterType === 'fecha_compra' ? new Date(a.fecha_compra) : (filterType === 'cantidad' ? a.cantidad : a.hora);
      const valueB = filterType === 'fecha_compra' ? new Date(b.fecha_compra) : (filterType === 'cantidad' ? b.cantidad : b.hora);
    ...
    if (searchText) {
        filteredData = filteredData.filter(item => 
          item.nombre.toLowerCase().includes(searchText.toLowerCase()) // Filtrar solo por nombre
        );
      }
      return filteredData;
  };
  ...
  const confirmDelete = async () => {
    try {
      const response = await fetch(`http://10.0.2.2:5000/api/sacos_comida/${itemToDelete}`, {
        method: 'DELETE',
      });
      if (!response.ok) {
    ...
    return (
      ...
      >
        <View style={styles.rowHeader}>
          <Text style={styles.rowText}>Tipo: {item.nombre}</Text>
        ...
        {isSelected && (
          <View style={styles.rowDetails}>
            ...
            <Text style={styles.detailText}>{item.fecha_compra}</Text>
            <Text style={styles.detailTitle}>Proveedor:</Text>
            <Text style={styles.detailText}>{item.proveedor || 'No disponible'}</Text>
            <Text style={styles.detailTitle}>Valor:</Text>
            <Text style={styles.detailText}>{item.precio || 'No disponible'}</Text>
...
export default NivelesComidaScreen;
