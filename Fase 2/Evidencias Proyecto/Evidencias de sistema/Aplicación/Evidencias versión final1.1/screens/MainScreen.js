...
export default function MainScreen({ navigation, route }) {
  ...
  useEffect(() => {
    ...
    if (currentRoute !== 'Main') {
      console.log('Evitando acciones no deseadas al no estar en Main.');
      return; // Salir si no estamos en la pantalla actual
    }
  
    (async () => {
      try {
        // Solicitar permisos y cargar ubicación
        ...
        if (sensorResponse.data.feeds && sensorResponse.data.feeds.length > 0) {
          setSensorData(sensorResponse.data.feeds[0]);
        } else {
          setErrorMsg('No se encontraron datos del sensor.');
        }
      } catch (error) {
        setErrorMsg('Ocurrió un error al cargar los datos.');
        console.error('Error en MainScreen:', error);
      }
    })();
  }, [navigation]);
...
const styles = StyleSheet.create({
  ...
  mapContainer: {
    width: '90%', // Ajustar el ancho para que coincida con los otros rectángulos
    height: 200, // Ajusta la altura según lo necesites
    borderRadius: 15, // Bordes redondeados
    overflow: 'hidden', // Asegurar que el mapa no se desborde de los bordes redondeados
    alignSelf: 'center', // Centrar el contenedor del mapa horizontalmente
    marginVertical: 10, // Espaciado vertical entre los elementos
    backgroundColor: '#FFFFFF', // Opcional: fondo blanco para consistencia
  },
  map: {
