...
import { View, Text, Image, StyleSheet, TouchableOpacity, FlatList, Alert, TextInput, Modal } from 'react-native';
...
function ComidaScreen() {
  ...
  const [stockModalVisible, setStockModalVisible] = useState(false); // Controla la visibilidad del modal
  ...
  const fetchComidaData = async () => {
    try {
      ...
      // Filtrar los sacos con cantidad menor o igual a 10
      const lowStock = tiposData.filter(tipo => tipo.cantidad <= 10);
      ...
      headerRight: () => (
        <TouchableOpacity onPress={() => setStockModalVisible(true)}>
          <View style={styles.notificationIconContainer}>
            <Icon name="bell" size={24} color="#384EA2" />
            {lowStockSacos.length > 0 && (
              <View style={styles.notificationBadge}>
                <Text style={styles.notificationText}>{lowStockSacos.length}</Text>
              </View>
            )}
          </View>
        </TouchableOpacity>
  ...
  return (
    <View style={styles.container}>
      ...
      <TouchableOpacity
        ...
        onPress={() => navigation.navigate('NivelesComida')} // Redirecciona a NivelesComidaScreen
      >
        <Text style={styles.nivelesButtonText}>Ver Niveles de Comida</Text>
      </TouchableOpacity>
...
<View style={styles.container}>
      ...
      {/* Modal para mostrar sacos con bajo stock */}
      <Modal
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={styles.modalContainer}>
            <Text style={styles.modalTitle}>Sacos con cantidad baja:</Text>
            <FlatList
              data={lowStockSacos}
              keyExtractor={(item) => item.id.toString()}
              renderItem={({ item }) => (
            ...
            />
            <TouchableOpacity onPress={() => setStockModalVisible(false)} style={styles.closeButton}>
...
const styles = StyleSheet.create({
  ...
  notificationIconContainer: {
    position: 'relative',
    marginRight: 20, // Ajusta este valor para mover la campana más a la izquierda
  },
  notificationBadge: {
    position: 'absolute',
    right: -8,
